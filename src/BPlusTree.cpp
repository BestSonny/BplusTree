...
Key BPlusTree<Key, Value>::innerNode::split(Node *newNode, Key k) {
  if (k > key[minKeyNum] && k < key[minKeyNum + 1]) {
    for (int i = 0; i < minKeyNum; i++) {
      ...
      ((innerNode *)newNode)->child[i + 1] =
          child[i + maxKeyNum - minKeyNum + 1];
    }
...
template <class Key, class Value>
void BPlusTree<Key, Value>::insertNode(Node *node, Key key, Value v, Node *p,
                                       stack<Node *> *parent) {
  Node *newNode;
  Key newKey;
...
multimap<Key, Value> BPlusTree<Key, Value>::getrange(Key key1, Key key2) {
  ...
  while (true) {
    ...
    if (flag) {
      if (leaf->left == NULL) break;
      leaf = leaf->left;
      i = leaf->keyNum - 1;
    } else {
      if (i == leaf->keyNum - 1) {
        if (leaf->right == NULL) break;
        leaf = leaf->right;
        i = 0;
      } else {
        i++;
  ...
  while (true) {
    for (; i < leaf->keyNum && leaf->key[i] >= key1 && leaf->key[i] <= key2;
         i++) {
